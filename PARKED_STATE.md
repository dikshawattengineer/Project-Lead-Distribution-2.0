...
| 3c | `11c_clear_unassigned_pool_visibility.sql` | Remove Unassigned from `pool_profiles` only (agents + managers) |
...
- **Far-future** CED (541+ days) → tag **UPSELLING** for QA; holding pool **Unassigned** after **11b**. **No apply** while parked.
- Run **11c** after **11b** so Unassigned is cleared from `pool_profiles` (no full **12** re-run needed).
